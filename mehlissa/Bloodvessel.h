...
* browses the nanobots of each stream in order of their positions and moves each Nanobot. If the resulting
* position exceeds the current bloodvessel the nanobot gets pushed to the next bloodvessel (and so on).
...
private:
  ...
  double m_fingerprintFormationTime; //out of csv, time that a message molecule needs to be formed after release from the nanobot
  bool m_hasActiveFingerprintMessage; //turns true after nanolocator was in vessel and timer of formation ended succesfully 
  ...
  double m_transitionto2; //probability blood flows to second vessel (if exists)
...
public:
  ...
  */
  ...
  /**
  * This function is called when the timer expires and the fingerprint is finished. 
  */
  void TimerCallback();

  /**
  * \returns true if fingerprint message molecules are active in the Bloodvessel.
  */ 
  bool isActive();

  /**
  * Checks if the Nanobot is of type nanocollector and in it's target organ. If the
  *  target organ has message molecules active, the collector collects them and turns
  *  it's tissue detected attribute to true. 
  * \param value list of Nanobots that need to get checked.  
  */
  void CheckCollect(list<Ptr<Nanobot>> nbToCheck); 

  /**
  * Checks if the Nanobot is of type Particle and if there are Nanobots in its range to detect it. If the
  * Particle is detected, its count goes up. 
  * \param value list of Nanobots that need to get checked.  
  */
