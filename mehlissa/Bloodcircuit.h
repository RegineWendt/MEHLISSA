...
private:
  ...
  */
  void InjectNanobots (int numberOfNanobots, int numberOfCollectors, int numberOfLocators, Ptr<Bloodvessel> bloodvessel, int particleMode);
...
public:
  ...
  Bloodcircuit (unsigned int numberOfNanobots, unsigned int numberOfCollectors, unsigned int numberOfLocators, unsigned int injectionVessel, Ptr<PrintNanobots> printer, unsigned int particleMode);

  /// Destructor to clean up the map.
  ~Bloodcircuit ();

  static int BeginSimulation (unsigned int simulationDuration, unsigned int numOfNanobots, unsigned int injectionVessel, unsigned int numOfCollectors, unsigned int numOfLocators, unsigned int particleMode);
};
}; // namespace ns3
#endif