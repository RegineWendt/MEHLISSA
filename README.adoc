...
The current implementation has been assessed against the complete MEHLISSA vision described in the literature and dissertation. The resulting analysis and the roadmap for a new, multi-layer MEHLISSA generation are available in the repository documentation:

* link:docs/IST_ANALYSE.md[Analysis of the current state (German)]
* link:docs/ROADMAP.md[Roadmap for a new MEHLISSA generation (German)]
* link:docs/README.md[Documentation index]

== Licensing

MEHLISSA uses licenses per file and artifact:

* independently developed MEHLISSA Next code: `MPL-2.0`
* historical implementations and direct ports: `GPL-2.0-only`
* new project-owned documentation and released project data: `CC-BY-4.0`

See link:LICENSE.md[the repository licensing policy] for the exact boundaries.
Existing datasets and third-party material are not relicensed without a
confirmed rights chain.
...
== Endocrine/AVS Extension

This repository includes an extended vascular transition setup for a clinically inspired endocrine signaling scenario based on adrenal venous sampling (AVS).

In the original MEHLISSA configuration, the vascular layout uses a simplified abdominal representation. For the endocrine/AVS scenario, we extend this layout by introducing separate vessel segments for the left and right adrenal glands, their corresponding adrenal veins, and their connections to the inferior vena cava (IVC).

These additions support a source-channel-receiver endocrine signaling model in which:
- the left and right adrenal glands act as transmitters,
- the vascular network acts as the propagation channel,
- and the left adrenal vein, right adrenal vein, and IVC act as passive receivers.

Hormones released from both adrenal glands propagate through the extended vascular network via diffusion and advection and are passively measured at the receiver sites in an AVS-inspired configuration.

For this scenario, use the extended transition file:
`vasculature_transitions_endocrine_avs.csv`

The original transition table is kept unchanged to preserve backward compatibility with the base MEHLISSA setup.

image::Images/endocrine_avs_layout.png[Endocrine AVS layout, width=500]

[caption=""]
.Figure: Extended endocrine/AVS layout with separate adrenal glands, adrenal veins, and IVC receiver configuration.
