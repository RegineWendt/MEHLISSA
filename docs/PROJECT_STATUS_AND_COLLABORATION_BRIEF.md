...
**Current product focus:** PCQ-1 pulmonary and capillary pre-outcome amendment and governed evidence acquisition
...
- The next scientific cycle has started with PCQ-1 design v0.1.0. It freezes
  ...
  No request has been sent and no candidate participant record acquired. This
  is prospective study discipline and governed source selection, not yet new
...
|---|---|---|
| Pulmonary and capillary qualification | **PCQ-1.1 and PCQ-1.2 completed locally:** two candidate hashes, four tracks, six primary endpoints, data-role separation, uncertainty/identifiability, controls, and a thirteen-source candidate register are machine-checked. Arizona iCPET remains the primary hemodynamic request; D'Souza 2025 is prioritized for joint functional volume and flow; Lassen 2023 requires a whole-pulmonary transit observation model; Bailey 2019 is the next five-lobe inquiry; PVDOMICS is not eligible as the initial healthy invasive source under its published protocol. PCQ-1.3 must lock observation models and tolerances before participant records. | Present result: auditable design and source selection, not a validation pass. Intended exit: bounded pulmonary and capillary claims evaluated without refitting, with uncertainty and all partial or negative findings retained. |
...
| `docs/qualification/PULMONARY_CAPILLARY_QUALIFICATION_PROTOCOL.md` and `PCQ1_EVIDENCE_SOURCE_SCREEN.md` | Active PCQ-1 claim, frozen candidates, endpoints, ranked sources, access/licence boundary, controls, and pre-outcome sequence. |
