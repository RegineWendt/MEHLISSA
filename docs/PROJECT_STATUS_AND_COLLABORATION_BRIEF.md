...
**Current product focus:** scientific qualification and external user feedback
...
|---|---|---|
| Evidence and validity baseline | Map every executable model, physiological parameter, assumption, calibration target, output, and claim to its source, licence, maturity, uncertainty, and validation status. | A machine-readable qualification inventory with no implicit scientific claim. |
| Pulmonary and capillary qualification | Use subject-level or otherwise joint data where possible, keep calibration and validation source-disjoint, and quantify parameter and structural uncertainty. | Bounded pulmonary and capillary claims evaluated with predeclared metrics and retained negative findings. |
