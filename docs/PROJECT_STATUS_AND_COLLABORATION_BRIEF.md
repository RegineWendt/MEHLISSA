...
**Current product focus:** PCQ-1.5 pulmonary and capillary uncertainty and identifiability
...
|---|---|---|
| Pulmonary and capillary qualification | **PCQ-1.1 through PCQ-1.4 completed locally:** design, thirteen-source register, four guarded source roles, eight observation models, sample/precision floors, six numeric gates, and statistical/blocked-state semantics are machine-checked. PCQ-1.4 adds a rights/provenance manifest, four strict normalized schemas/adapters, an outside-Git quarantine boundary, metadata-only output, and outcome-blind fixtures. It rejects unsafe input before opening measured data and keeps Lassen transit blocked. PCQ-1.5 next quantifies uncertainty, sensitivity, convergence, and identifiability while access remains pending. | Present result: auditable prospective design, source selection, analysis plan, and safe data boundary - not a validation pass. Intended exit: bounded pulmonary and capillary claims evaluated without refitting, with uncertainty and all partial or negative findings retained. |
...
| `docs/qualification/PULMONARY_CAPILLARY_QUALIFICATION_PROTOCOL.md`, `PCQ1_EVIDENCE_SOURCE_SCREEN.md`, `PCQ1_PRE_OUTCOME_AMENDMENT.md`, and `PCQ1_DATA_INGRESS.md` | Active PCQ-1 claim, frozen candidates and analysis, ranked sources, manifest-first rights checks, outside-Git quarantine, normalized adapters, controls, and pre-outcome sequence. |
