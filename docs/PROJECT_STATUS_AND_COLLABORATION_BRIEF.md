...
**Current product focus:** PCQ-1.4 rights-aware pulmonary and capillary data adapters
...
|---|---|---|
| Pulmonary and capillary qualification | **PCQ-1.1 through PCQ-1.3 completed locally:** two candidate hashes, four tracks, a thirteen-source register, four guarded source roles, eight observation models, sample/precision floors, six primary numeric gates, 90% equivalence intervals, data-role separation, controls, and blocked-state semantics are machine-checked. Arizona iCPET can initially provide only a bounded pilot because five controls cannot meet the ten-participant decision floor; D'Souza 2025 remains primary for functional volume and flow; Lassen 2023 cannot activate until independently fixed extra-capillary and mixing delays exist; Bailey 2019 remains the five-lobe inquiry. PCQ-1.4 next builds rights-aware schemas, quarantine, and outcome-blind adapters. | Present result: auditable prospective design, source selection, and analysis plan, not a validation pass. Intended exit: bounded pulmonary and capillary claims evaluated without refitting, with uncertainty and all partial or negative findings retained. |
...
| `docs/qualification/PULMONARY_CAPILLARY_QUALIFICATION_PROTOCOL.md`, `PCQ1_EVIDENCE_SOURCE_SCREEN.md`, and `PCQ1_PRE_OUTCOME_AMENDMENT.md` | Active PCQ-1 claim, frozen candidates, ranked sources, guarded observation models, sample/precision floors, numeric gates, access/licence boundary, controls, and pre-outcome sequence. |
