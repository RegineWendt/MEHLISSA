...
**Current product focus:** BCQ-1.2 prospective no-refit biological cell-model reproduction protocol
...
- BCQ-1.1 now provides a machine-checked four-candidate biological cell-model
  ...
  program. No external model has been imported or executed and M5 remains
  `software_test_surrogate`; BCQ-1.2 must freeze the no-refit reproduction
  protocol before trajectories are generated.
...
are indexed in `docs/qualification/README.md`; BCQ-1.2 is the current focus.
...
| Biological cell-model qualification | **BCQ-1.1 completed locally:** a machine-checked four-candidate screen selects the minimal Kallenberger 2014 CD95L-CD95-caspase-8 `BIOMD0000000523`/`0524` pair; exact public commits and hashes, CC0 model rights, original CD95-HeLa versus wild-type HeLa evidence roles, three alternatives, and average-cell/same-family/non-clinical limitations are explicit. BCQ-1.2 next freezes the independent external-solver protocol before import or execution. | Present result: auditable public-model and licence selection, not a reproduced model or new biological evidence. Intended exit: no-refit external-solver reproduction, typed M5 mapping, cross-engine and structural checks, justified population semantics, and independent review yielding one narrowly evidence-backed M5 variant. |
...
| `docs/qualification/BCQ1_BIOLOGICAL_CELL_MODEL_SELECTION.md` | BCQ-1.1 ranked public-model and licence screen, selected CD95L-CD95-caspase-8 artifacts, exact source identities, evidence roles, limitations, and the BCQ-1.2 through BCQ-1.7 sequence. |
