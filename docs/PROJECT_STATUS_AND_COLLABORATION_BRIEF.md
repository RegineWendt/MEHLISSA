...
**Current product focus:** PCQ-1 pulmonary and capillary qualification design and evidence acquisition
...
- The next scientific cycle has started with PCQ-1 design v0.1.0. It freezes
  ...
  and an amendment gate before any new validation outcome is inspected. This
  is prospective study discipline, not yet new physiological evidence.
...
|---|---|---|
| Pulmonary and capillary qualification | **Started with PCQ-1 design v0.1.0:** two candidate hashes, four tracks, six primary endpoints, data-role separation, uncertainty/identifiability, controls, and the non-clinical boundary are machine-checked. Next screen source-disjoint participant-level and joint flow-volume-transit evidence, then lock numeric tolerances before outcomes. | Present result: auditable pre-outcome design, not a validation pass. Intended exit: bounded pulmonary and capillary claims evaluated without refitting, with uncertainty and all partial or negative findings retained. |
...
| `docs/qualification/PULMONARY_CAPILLARY_QUALIFICATION_PROTOCOL.md` | Active PCQ-1 bounded claim, frozen candidates, endpoints, data rules, uncertainty plan, controls, and evidence-acquisition sequence. |
