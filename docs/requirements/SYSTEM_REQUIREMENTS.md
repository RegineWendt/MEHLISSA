...
| UX-014 | A supported graphical-workbench release shall provide a versioned installable Python package and console entry point, pass an isolated clean-install exercise, preserve an explicit executable/repository-artifact boundary, support documented novice and expert example tasks, and verify semantic keyboard, screen-reader, responsive, and error-recovery baselines on every supported platform. | N | P1 | T/I | RM UX-6.8; ADR-0050 |
