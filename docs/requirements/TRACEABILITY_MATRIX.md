...
| ORG-002 | DISS pp. 118–122 | PART | M3 | M2.4 checks whole-body perfusion; M3.7 adds source-scoped pressure, flow, PVR, compliance, transit, and right/left lung perfusion with uncertainty; M3.8–M3.13 add independent aggregate/multipoint evaluation and separately calibrated empirical flow/age refinements; M3.14 adds a mutually exclusive Linehan law and retains its no-refit 11/15 result; M3.15 uses an independent older `alpha`, reduces older RMSE from 5.411 to 4.603 mmHg, and preserves the unchanged 11/15 result rather than fitting Wolsk |
...
| DATA-006 | RM 6.2 | PART | M0 | M3.7 stores calibration/validation/derived roles separately; M3.8–M3.13 enforce source separation, overlap accounting, exact aggregate-statistic semantics, and frozen comparisons; M3.14 repeats the locked validation for a structural alternative; M3.15 adds explicit standard-error metadata and an independently calibrated older coefficient while preserving the inferior frozen result; participant-level data and release-wide enforcement follow |
