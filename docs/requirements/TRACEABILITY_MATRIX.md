...
| CAP-006 | DISS pp. 129, 154 | PART | M4 | M4.8 defines a typed implementation-neutral request/response, loads a strict evidence-scoped profile, constructs an analytical free-diffusion adapter, and derives a bounded reference request from the pulmonary capillary card; independent particle/surrogate comparison, advection, reactions, noise, and physiological signal qualification follow |
