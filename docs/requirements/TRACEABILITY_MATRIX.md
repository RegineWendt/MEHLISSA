...
| ORG-001 | DISS pp. 118–126 | PART | M3 | coarse and serial-region surrogates plus resting, bounded flow-adaptive, age-conditioned, invasively young-resistance-conditioned, and pressure-distensible literature-parameterized mean pulmonary 0D model cards and tests exist; anatomical, pulsatile, and regional activity-state variants follow |
| ORG-002 | DISS pp. 118–122 | PART | M3 | M2.4 checks whole-body perfusion; M3.7 adds source-scoped pressure, flow, PVR, compliance, transit, and right/left lung perfusion with uncertainty; M3.8 independently validates aggregate pressure/compliance/resting-RC compatibility; M3.9 adds source-disjoint bounded PVR/compliance flow adaptation and reduces the independent exercise RC discrepancy without hiding its remaining failure; M3.10 software-verifies pseudonymous subject-level analysis; M3.11 evaluates four independent published population pressure-flow series without refitting; M3.12 uses a separate Kane age calibration and improves the untouched result from 10/18 to 14/18; M3.13 identifies the young PVR level with invasive calibration and reaches 15/15 in the disjoint Wolsk cohort while excluding an overlapping historical summary; M3.14 adds a mutually exclusive Linehan structural law and retains its no-refit 11/15 result and older-stratum failure |
...
| ORG-005 | DISS pp. 123–126 | PART | M3 | schema-selected coarse, three-region surrogate, and pulmonary 0D implementations share one component contract; empirical and Linehan pressure-distensible 0D laws are independently selectable and cannot be double-counted; anatomical 1D/geometry refinement follows |
...
| DATA-006 | RM 6.2 | PART | M0 | M3.7 stores calibration/validation/derived roles separately; M3.8 adds executable source-disjointness and independent aggregate validation; M3.9 audits cohort overlap and keeps Claessen calibration separate from Bentley validation; M3.10 adds a minimum-three-stage pseudonymous schema and synthetic-evidence guard; M3.11 adds a separate published-population schema, exact mean/SD versus mean/95% CI semantics, non-overlapping cohort accounting, explicit cardiac-index derivation, and a no-refit partial result; M3.12 records representative age and frozen before/after results; M3.14 repeats the locked validation for a structural alternative and preserves its inferior older-stratum result; participant-level data and release-wide enforcement follow |
