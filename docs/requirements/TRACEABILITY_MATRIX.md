...
| CAP-006 | DISS pp. 129, 154 | DONE | M4 | M4.8 defines a typed implementation-neutral request/response and analytical free-diffusion adapter; M4.9 adds a deterministic Brownian endpoint adapter; M4.10 adds bounded fixed-step particle paths, reflecting-box software support, and predeclared analytical, refinement, and `6Dt` gates on the same pulmonary-bound reference request; mesoscopic fields, anatomical surfaces, external tools, advection, reactions, noise, and physiological signal qualification follow without reopening the stable contract |
