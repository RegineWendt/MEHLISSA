...
| DATA-005 | RM 2.5 | PART | M0/M1 | the Paper 1 evidence matrix enforces evidence class, intended use, population/state, sources, licences, uncertainty/ranges, validity, limitations, blockers, and linked artifacts across all six current executable families; equivalent enforcement for every future model and every individual parameter follows |
| DATA-006 | RM 6.2 | PART | M0 | the Paper 1 matrix and source-role audit make calibration/validation/verification roles and overlap explicit across six families; M3.8–M3.17 retain source-disjoint and negative pulmonary findings; participant-level data and independent qualification outside the strongest lung comparisons still follow |
...
| DATA-008 | RM 6.2 | PART | M5/M7 | stochastic receptor population moments, Wilson classification intervals, particle-channel comparisons, campaign replicate summaries, and Paper 1 predeclared conditions/seeds/repetitions/tolerances use declared statistical semantics; release-wide power/design rules and broad uncertainty propagation remain open |
