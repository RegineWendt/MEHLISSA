...
| CAP-006 | DISS pp. 129, 154 | DONE | M4 | M4.8 defines a typed implementation-neutral request/response and analytical free-diffusion adapter; M4.9 adds a deterministic Brownian endpoint adapter; M4.10 adds bounded fixed-step particle paths and reflecting-box software support; M4.11 adds a conservative radial finite-volume field with complete spatial output, explicit degradation and boundary-loss accounting, and 128-to-256-shell analytical/refinement gates on the same pulmonary-bound request; anatomical surfaces, external tools, advection, richer reactions, noise, and physiological signal qualification follow without reopening the stable contract |
