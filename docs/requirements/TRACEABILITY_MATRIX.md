...
| CAP-006 | DISS pp. 129, 154 | DONE | M4 | M4.8 defines a typed implementation-neutral request/response and analytical free-diffusion adapter; M4.9 adds an independently implemented deterministic Brownian endpoint adapter and passes predeclared count, standardized-error, and relative-error gates on the same pulmonary-bound reference request; richer trajectories, external tools, advection, reactions, noise, and physiological signal qualification follow without reopening the stable software contract |
