...
| ORG-002 | DISS pp. 118–122 | PART | M3 | M2.4 checks whole-body perfusion; M3.7 adds source-scoped pressure, flow, PVR, compliance, transit, and right/left lung perfusion with uncertainty; M3.8 independently validates aggregate pressure/compliance/resting-RC compatibility; M3.9 adds source-disjoint bounded PVR/compliance flow adaptation and reduces the independent exercise RC discrepancy without hiding its remaining failure; M3.10 software-verifies pseudonymous subject-level analysis; M3.11 evaluates four independent published population pressure-flow series without refitting and exposes age-stratified failures (10/18 stages agree) |
