...
| Pulmonary and capillary qualification | **PCQ-1.1 through PCQ-1.3 complete locally:** [design v0.1.0 and amendment v0.2.0](qualification/PULMONARY_CAPILLARY_QUALIFICATION_PROTOCOL.md) freeze the entering candidates, bounded claim, source roles, four tracks, eight observation models, six primary numeric gates, precision floors, data separation, uncertainty rules, controls, and blocked states before participant-level outcomes. PCQ-1.4 next implements rights-aware schemas, quarantine, and outcome-blind adapters. | The design, source-register, and amendment checkers pass. Scientific exit still requires governed source access, no-refit evaluation on source-disjoint data, full uncertainty and identifiability work, and independent review with all negative or partial findings retained. |
...
| Current qualification cycle | PCQ-1 pulmonary-capillary design and evidence acquisition, followed by the first biological cell-model candidate | Existing mechanisms gain bounded, independently testable scientific claims; the PCQ-1 design is frozen before new outcomes, while physiological qualification remains open |
...
24. Qualify pulmonary and capillary physiology. Use subject-level or otherwise
    ...
    - PCQ-1.4 through PCQ-1.7 implement strict data adapters, quantify
      uncertainty and identifiability, run the frozen no-refit evaluation, and
      publish an independently reviewed qualification report including all
      partial and failed findings.
