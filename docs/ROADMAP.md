...
| Biological cell-model qualification | **BCQ-1.1 through BCQ-1.3 complete locally:** the [candidate and licence screen](qualification/BCQ1_BIOLOGICAL_CELL_MODEL_SELECTION.md) selects the minimal Kallenberger 2014 CD95L-CD95-caspase-8 pair `BIOMD0000000523`/`0524`; the [prospective protocol](qualification/BCQ1_REPRODUCTION_PROTOCOL.md) and disclosed [replay amendment](qualification/BCQ1_REPRODUCTION_PROTOCOL_AMENDMENT_1.md) freeze source cases, COPASI 4.46 Build 300/LSODA, unresolved units, six runs, gates, controls, and failure retention; the [external-solver result](qualification/BCQ1_EXTERNAL_SOLVER_REPRODUCTION.md) passes nine unblocked computational gates and ten negative controls. | Present result: unchanged source artifacts independently execute with numerically equivalent replay and tightened-solver stability. Publication-curve alignment remains blocked, no MEHLISSA adapter exists, and biological qualification is not established. BCQ-1.4 next maps the model through typed contracts; cross-engine/structural checks, explicit population limits, and independent review still follow before one M5 variant can move beyond `software_test_surrogate`. |
| Dynamic capillary-tissue-cell coupling | Replace the prescribed homogeneous snapshot with a time-resolved field including transport, binding or consumption, and explicit cross-layer uncertainty. | The cell exposure is generated by the coupled transport model and verified for balance, convergence, timing, and evidence compatibility. |
...
| Current qualification cycles | PCQ-1 pulmonary-capillary work is access-pending after its outcome-blind design and outreach package; BCQ-1.1 through BCQ-1.3 have selected, protocolled, and independently reproduced the first public biological model family in COPASI, with publication alignment blocked; BCQ-1.4 typed MEHLISSA mapping is active | Existing mechanisms gain bounded, independently testable scientific claims without forcing human-data access to block public-model reproduction; external source execution is now demonstrated, while biological qualification still requires mapping, cross-engine comparison, population decisions, and review |
...
25. Qualify at least one biological cell-model variant. Replace synthetic-only
    ...
    - BCQ-1.4 map only declared inputs, states, time/unit semantics, and outputs
      through a typed M5 adapter without refitting or rewriting the source
      mechanism.
    ...
    - BCQ-1.6 add population/uncertainty behavior only if the ensemble
      ...
      average-cell limitation explicitly.
    - BCQ-1.7 complete independent scientific, licence, code-to-equation,
      archive, claim, User Guide, Roadmap, and status review.
