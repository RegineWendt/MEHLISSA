...
| Dynamic capillary-tissue-cell coupling | **DCCQ-1.1 complete:** the [Dynamic Capillary-Tissue-Cell Qualification plan](qualification/DCCQ1_QUALIFICATION_PLAN.md) freezes a bounded intended use, audits six existing component baselines, requires one chemically consistent ligand and a seven-owner amount ledger, defines five evidence levels, eight blocked qualification gates, ten negative controls, six uncertainty classes, and a seven-increment programme. It explicitly prevents the non-consuming M5.2 snapshot or the unit-unresolved fixed-input BCQ model from being relabelled as dynamic biological coupling. DCCQ-1.2 will rank a named ligand-receptor-cell-context candidate and alternatives by artifact identity, unit compatibility, data roles, reuse rights, and independently usable observations before equations are frozen. | Current exit: the prospective design and compatibility audit is machine checked, not the dynamic coupling. Scientific exit still requires DCCQ-1.2 through DCCQ-1.7: source selection, a pre-output equation/evaluation protocol, typed consumptive implementation, balance/convergence/timing qualification, uncertainty and source-disjoint comparison, and independent bounded review. |
...
26. Implement and qualify dynamic capillary-tissue-cell coupling through the
    **Dynamic Capillary-Tissue-Cell Qualification (DCCQ-1)** programme.
    ...
    - DCCQ-1.2 select and licence-screen one named ligand-receptor-cell-context
      ...
      reassess VEGF-A/VEGFR without preselecting it.
    - DCCQ-1.3 freeze the state equations, parameter roles, synchronization,
