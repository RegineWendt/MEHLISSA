...
| Pulmonary and capillary qualification | **Started:** [PCQ-1 design v0.1.0](qualification/PULMONARY_CAPILLARY_QUALIFICATION_PROTOCOL.md) freezes the entering lung and capillary candidates, bounded claim, four tracks, six primary endpoints, data separation, uncertainty plan, controls, and amendment rules before new outcomes. Next obtain participant-level multipoint pulmonary data and, where possible, jointly measured flow, capillary volume, transit, hematocrit, and dynamic regional-state evidence. | The design checker passes now; scientific exit still requires locked numeric tolerances followed by no-refit evaluation on source-disjoint data with uncertainty and retained negative findings. |
