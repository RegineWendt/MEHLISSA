...
| Biological cell-model qualification | **BCQ-1.1 and BCQ-1.2 complete locally:** the [candidate and licence screen](qualification/BCQ1_BIOLOGICAL_CELL_MODEL_SELECTION.md) selects the minimal Kallenberger 2014 CD95L-CD95-caspase-8 pair `BIOMD0000000523`/`0524`; the [prospective protocol](qualification/BCQ1_REPRODUCTION_PROTOCOL.md) binds source cases, COPASI 4.46 Build 300/LSODA, unresolved unit semantics, a six-run grid, observables, invariants, numeric rules, ten negative controls, and a failure-retaining archive before execution. | Present result: an auditable model-family, rights, and pre-trajectory execution decision, not a reproduced model or biological evidence. BCQ-1.3 next executes the independent solver; typed MEHLISSA mapping, cross-engine/structural checks, explicit population limits, and independent review still follow before one M5 variant can move beyond `software_test_surrogate`. |
...
| Current qualification cycles | PCQ-1 pulmonary-capillary work is access-pending after its outcome-blind design and outreach package; BCQ-1.1 has selected and licence-screened the first biological model family, BCQ-1.2 has frozen its no-refit reproduction rules without generating a trajectory, and BCQ-1.3 is the active external-solver execution step | Existing mechanisms gain bounded, independently testable scientific claims without forcing human-data access to block public-model reproduction; neither protocol design nor model selection is itself a validation result |
...
25. Qualify at least one biological cell-model variant. Replace synthetic-only
    ...
    - ~~BCQ-1.1 select and license-screen the first public model family.~~
      ...
      boundaries. No external model has been imported or executed.
