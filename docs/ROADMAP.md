...
| Pulmonary and capillary qualification | **PCQ-1.1 through PCQ-1.4 complete locally:** [design v0.1.0 and amendment v0.2.0](qualification/PULMONARY_CAPILLARY_QUALIFICATION_PROTOCOL.md) freeze candidates, claims, source roles, observation models, numeric gates, precision and uncertainty rules; [rights-aware ingress](qualification/PCQ1_DATA_INGRESS.md) adds manifest-first authorization, outside-Git quarantine, four strict normalized adapters, metadata-only output, and outcome-blind fixtures. PCQ-1.5 next addresses uncertainty and identifiability. | Design, source-register, amendment, and ingress checkers pass without participant outcomes. Scientific exit still requires governed access, complete uncertainty/identifiability work, no-refit source-disjoint evaluation, and independent review retaining negative and partial findings. |
...
24. Qualify pulmonary and capillary physiology. Use subject-level or otherwise
    ...
    - PCQ-1.5 through PCQ-1.7 quantify uncertainty and identifiability, run the
      frozen no-refit evaluation, and publish an independently reviewed
      qualification report including all partial and failed findings.
