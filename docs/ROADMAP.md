...
| Biological cell-model qualification | **BCQ-1.1 complete locally:** the [candidate and licence screen](qualification/BCQ1_BIOLOGICAL_CELL_MODEL_SELECTION.md) selects the minimal Kallenberger 2014 CD95L-CD95-caspase-8 pair `BIOMD0000000523`/`0524`, freezes public commits and hashes, separates CC0 model rights from article/data rights, ranks three alternatives, and records the average-cell and same-publication limitations before import. BCQ-1.2 next freezes a no-refit external-solver reproduction protocol. | Present result: an auditable model-family and rights decision, not biological evidence. Exit still requires independent solver reproduction, typed MEHLISSA mapping, cross-engine/structural checks, explicit population limits, and independent review before one M5 variant can move beyond `software_test_surrogate`. |
...
25. Qualify at least one biological cell-model variant. Replace synthetic-only
    ...
    - BCQ-1.2 freeze the prospective no-refit reproduction protocol before
      execution: solver/version, input artifacts, units, time grid, ligand
      conditions, observables, numeric tolerances, negative controls, result
      archive, and failure rules.
