...
Simulator::Simulator(int parallelity, double timeStep,
...
int Simulator::Simulate(uint64_t numberOfSeconds) {
    if (m_parallelity > 1)
        cout << "No parallel execution implemented, yet! Simulating "
                "sequentially."
             << endl;
...
int Simulator::SimulateSequential(uint64_t numberOfSeconds) {
    while (m_nextSteps.size() > 0 &&
           GlobalTimer::NowInSeconds() <= numberOfSeconds) {
        ...
        while (m_currentSteps.size() > 0) {
            ...
            shared_ptr<BloodVessel> bv_next =
                bv->Step(GlobalTimer::NowInSeconds());
            // if (bv_next != nullptr)
...
void Simulator::m_currentStepsSafeAppendRange(
    list<shared_ptr<BloodVessel>> bvs) {
    const std::lock_guard<std::mutex> lock(m_currentSteps_mutex);
    for (auto bv : bvs)
        m_currentSteps.push_back(bv);
}

void Simulator::m_transferStepsSafeAppendRange(
    list<shared_ptr<BloodVessel>> bvs) {
    const std::lock_guard<std::mutex> lock(m_transferSteps_mutex);
    for (auto bv : bvs)
